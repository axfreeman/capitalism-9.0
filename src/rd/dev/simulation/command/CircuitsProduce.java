		...
		for (Circuit c : circuits) {
			...
			for (Stock s : stocks) {
				if (!s.getStockType().equals("Productive")) {
					Dialogues.alert(logger,String.format("Non-productive stock of type [%s] called [%s] included as input ", s.getStockType(), s.getUseValueName()));
				}
			...
			Reporter.report(logger, 2, "  The sales stock of [%s] has grown to %.2f, its value to $%.2f (intrinsic value %.2f) and its price to $%.2f (intrinsic value %.2f)",
							c.getProductUseValueType(), salesStock.getQuantity(), salesStock.getValue(),salesStock.getValue()/melt, salesStock.getPrice(),salesStock.getPrice()/melt);
